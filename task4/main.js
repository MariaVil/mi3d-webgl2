...
window.addEventListener("mousemove", e => {
    if (is_active) {
        if (e.clientX - mouseDownX > 0) { // right
            settings.black-=step
            settings.white+=step
        } else if (e.clientX - mouseDownX < 0) { // left
            settings.black+=step
            settings.white-=step
        }
        if (e.clientY - mouseDownY > 0) { //down
            settings.black-=step
            settings.white-=step
        } else if (e.clientY - mouseDownY < 0) { // up
            settings.black+=step
            settings.white+=step
        }
        mouseDownX = e.clientX
        mouseDownY = e.clientY
        if (settings.black < minValue) settings.black = minValue
        if (settings.black > maxValue) settings.black = maxValue
